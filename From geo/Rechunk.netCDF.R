...
# Rechunk dimensions and convert the  time dimension from unlimited to fixed.
  ...
  in.path <- "/projectnb/dietzelab/pecan.data/input/NARR_CF/"
  prefix <- "NARR."
  out.path <- "/projectnb/dietzelab/pecan.data/input/NARR_CF_Rechunk/"
  ...
  for(i in 1:length(files)){    
    ...
    if(file.exists(infile)==TRUE && file.exists(outfile)==FALSE){
      ...
      system(paste0("ncpdq --no_tmp_fl -h -O -a y,x,time ",outfile," ",outfile))
#       system(paste0("rm -f ",tempfile))
    }
  }